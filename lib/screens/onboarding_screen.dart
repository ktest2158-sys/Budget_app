...
class _OnboardingScreenState extends State<OnboardingScreen> {
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildFeatureItem(Icons.calendar_today, 'Fortnight-based budgeting'),
                _buildFeatureItem(Icons.repeat, 'Recurring income & expenses'),
                _buildFeatureItem(Icons.pie_chart, 'Visual spending analytics'),
                _buildFeatureItem(Icons.savings, 'Savings tracking & buckets'),
              ],
            ),
          ),
        ],
  ...
  Widget _buildSettingsPage() {
    return ListView(
      padding: const EdgeInsets.all(24),
      children: [
        const Text('Configure Your Budget',
            style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Text('Set up your budget rules and preferences',
            style: TextStyle(color: Colors.grey[600])),
        const SizedBox(height: 32),
        Card(
          child: ListTile(
            ...
            onTap: () async {
              ...
              if (picked != null) setState(() => _fortnightStart = picked);
            },
          ),
        ),
        const SizedBox(height: 16),
  ...
  Widget _buildIncomePage() {
    return ListView(
      padding: const EdgeInsets.all(24),
      children: [
        const Text('Add Recurring Income',
            style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Text('Add your regular income sources',
            style: TextStyle(color: Colors.grey[600])),
        const SizedBox(height: 24),
        ElevatedButton.icon(
          onPressed: _addIncomeTemplate,
          ...
          ..._incomeTemplates.asMap().entries.map((entry) {
            ...
            return Card(
              child: ListTile(
                ...
                title: Text(income['name']),
                subtitle: Text(
                    '${income['category']} · \$${income['amount'].toStringAsFixed(2)} (${(income['frequency'] as Frequency).label})'),
                trailing: IconButton(
                  icon: const Icon(Icons.delete, color: Colors.red),
                  onPressed: () =>
                      setState(() => _incomeTemplates.removeAt(index)),
    ...
    return ListView(
      ...
      children: [
        const Text('Add Recurring Expenses',
            style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Text('Add your regular bills and expenses',
            style: TextStyle(color: Colors.grey[600])),
        const SizedBox(height: 24),
        ElevatedButton.icon(
          onPressed: _addExpenseTemplate,
        ...
        if (_expenseTemplates.isEmpty)
          ...
          ..._expenseTemplates.asMap().entries.map((entry) {
            ...
            return Card(
              child: ListTile(
                leading: Icon(
                  expense['isSavings'] == true
                      ? Icons.savings
                      : Icons.receipt_long,
                  color: expense['isSavings'] == true
                      ? Colors.indigo
                      : Colors.red,
                ),
                title: Text(expense['name']),
                subtitle: Text(
                    '${expense['category']} · \$${expense['amount'].toStringAsFixed(2)} (${(expense['frequency'] as Frequency).label})'),
                trailing: IconButton(
                  icon: const Icon(Icons.delete, color: Colors.red),
                  onPressed: () =>
                      setState(() => _expenseTemplates.removeAt(index)),
  ...
  Future<void> _addIncomeTemplate() async {
    ...
    if (result != null) setState(() => _incomeTemplates.add(result));
  }
  ...
  Future<void> _addExpenseTemplate() async {
    ...
    if (result != null) setState(() => _expenseTemplates.add(result));
  }
  ...
  Future<void> _finishOnboarding() async {
    ...
    for (var expense in _expenseTemplates) {
      await StorageService.addExpense(
        ...
        frequency: expense['frequency'],
        isTemplate: true,
        isSavings: expense['isSavings'] ?? false, // ✅ Pass savings flag
      );
    }
...
class _QuickExpenseDialogState extends State<_QuickExpenseDialog> {
  ...
  String? _category;
  Frequency _frequency = Frequency.fortnightly;
  bool _isSavings = false; // ✅ Savings toggle in onboarding too
  late List<String> _categories;
          ...
          onPressed: () {
            if (_formKey.currentState!.validate()) {
              Navigator.pop(context, {
                ...
                'amount': double.parse(_amountController.text),
                'frequency': _frequency,
                'isSavings': _isSavings, // ✅ Pass savings flag
              });
            }
          },
          child: const Text('Add'),
